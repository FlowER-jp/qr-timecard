// Night shift hours: 22:00-05:00 JST (Japanese Labor Standards Act)
// Premium rate: +25% for night hours
...
function calcNightMinutesGross(clockIn: Date, clockOut: Date): number {
  ...
  for (let day = startDay; day <= endDay; day++) {
    // Segment A: 00:00-05:00
    const segA = [day * 1440, day * 1440 + 5 * 60];
    // Segment B: 22:00-24:00
...
export function calcPayrollBreakdown(
  ...
  const normalMinutes = netMinutes - nightMinutes;
  ...
  return {
    ...
    baseAmount,
    nightPremiumAmount,
    totalBeforeIncentive: baseAmount + nightPremiumAmount,
  };
}
